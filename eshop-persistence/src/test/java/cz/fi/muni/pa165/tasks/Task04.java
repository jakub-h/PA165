...
public class Task04 extends AbstractTestNGSpringContextTests {
	...
	public void groupByAndOrderBySelectNew() {
		List<ColorCount> colorCounts = em.createQuery(
				"select new cz.fi.muni.pa165.tasks.ColorCount(p.color,count(p))" +
						"from Product p group by p.color order by p.color",
				ColorCount.class).getResultList();
	...
	public void findProductsAddedYesterday() {
		...
		List<Product> products= em.createQuery("from Product p where p.addedDate = :date",Product.class)
	...
	public void leftJoinCagesWithPets() {
		...
		List<ProductAndCategory> productAndCategory = em.createQuery(
				"select new cz.fi.muni.pa165.tasks.ProductAndCategory(p,c) " +
						"from Product p left join p.categories c on c.name= 'Kitchen' " +
						"order by p.name, c.name",ProductAndCategory.class).getResultList();
